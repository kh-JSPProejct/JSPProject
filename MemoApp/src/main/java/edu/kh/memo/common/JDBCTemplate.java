...
public class JDBCTemplate {
	...
	public static Connection getConnection() {
		...
		try {
			...
			// JDBCTemplate.class.getResource() : * 클래스패스(classpath) 내에서 지정된 리소스 파일을 찾는 메서드
			
			// * classpath란?
			// Java 프로그램이 클래스를 찾기 위해 검색하는 경로(위치)를 의미
			// -> (src/main/resources 또는 WEB-INF/classes에서 찾음)
			
			// getPath() : URL 객체에서 실제 파일 시스템의 경로(String 타입) 절대 경로 얻어옴
			...
			// (src/main/resources 폴더 기준으로 경로 작성)
		 	...
		 	// src/main/resources : 
		 	// 주로 애플리케이션 설정 파일, 메시지 프로퍼티 파일, 템플릿 파일, 정적 리소스 등을 포함하는 디렉터리
